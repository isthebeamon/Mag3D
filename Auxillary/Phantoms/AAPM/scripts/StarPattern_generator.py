...
r_maj_cm = %R_MAJ_CM%  # cm, major radius
r_min_cm = %R_MIN_CM%  # cm, minor radius
t_cm = %T_CM%  # cm, height
sector_angle = %SECTOR_ANGLE% # degrees, angle of each individual sector
num_sectors = %NUM_SECTORS% # Number of sectors in the star pattern. Evenly distributes sectors about 90, 180, 270
                            # and 360-degree locations, so num_sectors % 4 == 0. If sector_angle * num_sectors = 360
                            # then the star pattern is a full star pattern.
compress_option = %COMPRESS_OPTION%
vox_option = %VOX_OPTION%

# Error Handling
# if num_sectors % 4 != 0:
    # raise ValueError("The number of sectors must be divisible by 4.")

# Dimensions
r_maj = round(r_maj_cm / voxel_size)
r_min = round(r_min_cm / voxel_size)
dimX = int((r_maj_cm * 2) / voxel_size)
dimY = int((r_maj_cm * 2) / voxel_size)
dimZ = int(t_cm / voxel_size)
# if sector_angle * (2*num_sectors) == 360:
    # print("This script creates the full Star Pattern-style phantom.")
# else:
    # print("This script creates the partial Star Pattern-style phantom.")
...
phantom_shape = (dimZ, dimX, dimY)
phantom = np.zeros(phantom_shape, dtype=np.uint8)
phantom_center = (round(dimX / 2), round(dimY / 2))

# Create major circular shape of Star Pattern and add sectors
angle_step = np.deg2rad(sector_angle)
angles = np.linspace(0, 2 * np.pi, num_sectors, endpoint=False)
for i, angle in enumerate(angles):
    for x in range(dimX):
        for y in range(dimY):
            if (x - phantom_center[0]) ** 2 + (y - phantom_center[1]) ** 2 <= r_maj ** 2:
                # Calculate the angle of the current point
                point_angle = np.arctan2(y - phantom_center[1], x - phantom_center[0])
                if point_angle < 0:
                    point_angle += 2 * np.pi
                # Check if the point is within the current sector
                if np.abs(point_angle - angle) <= angle_step / 2:
                    if i % 2 == 0:
                        phantom[:, x, y] = intensity_Pb
                    else:
                        phantom[:, x, y] = intensity_air
            if r_min != 0.0:
                if (x - phantom_center[0]) ** 2 + (y - phantom_center[1]) ** 2 <= r_min ** 2:
                    phantom[:, x, y] = intensity_Pb
# Create copy of 0-180 degrees and flip it into 180-360 degree space to avoid visual error at 270 degrees
phantom[:, dimX // 2:, :] = np.rot90(np.flipud(phantom[:, :dimX // 2, :]), 2, axes=(0, 1))
...
output_file1 = f"StarPattern_sectors{int(sector_angle)}degree_nums{int(num_sectors)}_vox{int(voxel_size * 10000)}um.raw"
...
if vox_option == 'yes' and compress_option == 'yes':
    ...
    vox_filename = (f"StarPattern_sectors{int(sector_angle)}degree_nums{int(num_sectors)}_"
                    f"vox{int(voxel_size * 10000)}um.vox.gz")
    ...
    start_timer4 = time.time()
    vox_filename = (f"StarPattern_sectors{int(sector_angle)}degree_nums{int(num_sectors)}_"
                    f"vox{int(voxel_size * 10000)}um.vox")
    vox_file_path = os.path.join(output_folder1, vox_filename)
...
if compress_option == 'yes':
    ...
    output_file2 = (f"StarPattern_sectors{int(sector_angle)}degree_nums{int(num_sectors)}_"
                    f"vox{int(voxel_size * 10000)}um.raw.gz")
