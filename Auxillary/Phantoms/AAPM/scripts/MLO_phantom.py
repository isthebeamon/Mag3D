...
print("This script creates the MLO phantom.")
...
for x in range(volume_shape[1]):
    for y in range(volume_shape[2]):
        for z in range(volume_shape[0]):
            ...
            # Check if the voxel is within the interior
            if distance_from_center <= (semiellipse_semimajor_max - round(shield_cm / voxel_size)):
                # Create alternating pattern of adipose and glandular tissue
                ...
                if uniformity == 'heterogeneous':
                    if random.random() < glandularity / 100.0:
                        ...
                        volume[z, x, y] = intensity_adip
                if z < round(shield_cm / voxel_size):
                    volume[z, x, y] = intensity_adip
                if z >= (volume_shape[0] - round(shield_cm / voxel_size)):
                    volume[z, x, y] = intensity_adip
            # Check if the voxel is within the outer adipose layer
            elif distance_from_center <= semiellipse_semimajor_max:
                volume[z, x, y] = intensity_adip

# Create MLO-like structures in upper half of phantom volume
vz_MLO = vz
vx_MLO = round(vx / 2)
vy_MLO = vy
volume_upper_shape = (vz_MLO, vx_MLO, vy_MLO)
volume_upper = np.zeros(volume_upper_shape, dtype=np.uint8)
for x in range(volume_upper_shape[1]):
    for y in range(volume_upper_shape[2]):
        for z in range(volume_upper_shape[0]):
            # Pectoralis
            if y <= (volume_upper_shape[2] - x) / 2:
                volume_upper[z, x, y] = intensity_musc
                if shield_cm != 0.0:
                    volume_upper[0:round(shield_cm / voxel_size), x, y] = intensity_adip
                    volume_upper[vz_MLO - round(shield_cm / voxel_size):vz_MLO, x, y] = intensity_adip
            # Breast Tissue
            if y >= (volume_upper_shape[2] - x) / 2:
                if uniformity == 'homogeneous': # ONLY WORKS FOR 50, 25, 12.5, etc., NEED TO FIX THIS
                    if (x + y + z) % (100.0 / glandularity) == 0:
                        volume_upper[z, x, y] = intensity_glan
                    else:
                        volume_upper[z, x, y] = intensity_adip
                if uniformity == 'heterogeneous':
                    if random.random() < glandularity / 100.0:
                        volume_upper[z, x, y] = intensity_glan
                    else:
                ...
                if shield_cm != 0.0:
                    volume_upper[0:round(shield_cm / voxel_size), x, y] = intensity_adip
                    volume_upper[vz_MLO - round(shield_cm / voxel_size):vz_MLO, x, y] = intensity_adip
            # Breast Tissue - Glandular
            #if y >= (volume_upper_shape[2] - x) / 2 and (x + y + z) % 2 != 0:
                #volume_upper[z, x, y] = intensity_glan
                #if shield_cm != 0.0:
                    #volume_upper[0:round(shield_cm / voxel_size), x, y] = intensity_adip
                    #volume_upper[vz_MLO - round(shield_cm / voxel_size):vz_MLO, x, y] = intensity_adip
            # Air
            if y > (volume_upper_shape[2] + x) / 2:
                volume_upper[z, x, y] = intensity_air
                if shield_cm != 0.0:
                    volume_upper[0:round(shield_cm / voxel_size), x, y] = intensity_air
                    volume_upper[vz_MLO - round(shield_cm / voxel_size):vz_MLO, x, y] = intensity_air
            # Bounding Skin Line
            if (y > 0 and volume_upper[z, x, y] == intensity_air and volume_upper[z, x, y - 1] != intensity_air
                    and shield_cm != 0.0):
                # Transform the non-air voxel and specified number of voxels before it
                volume_upper[z, x, y - round(shield_cm / voxel_size):y] = intensity_adip
            if shield_cm != 0.0:
                volume_upper[z, vx_MLO - 2, vy_MLO - round(shield_cm / voxel_size):] = intensity_adip
                volume_upper[z, vx_MLO - 1, vy_MLO - round(shield_cm / voxel_size):] = intensity_adip
volume[:, 0:vx_MLO, :] = volume_upper
...
output_file1 = (f"MLO_phantom_vox{int(voxel_size_um)}um_thick{int(d_cm)}cm_extentML{int(vx_cm)}cm_"
                f"extentPA{int(vy_cm)}cm_shield{int(shield_mm)}mm_paddle{int(paddle_thickness_mm)}mm_"
...
if vox_option == 'yes' and compress_option == 'yes':
    ...
    vox_filename = (f"MLO_phantom_vox{int(voxel_size_um)}um_thick{int(d_cm)}cm_extentML{int(vx_cm)}cm_"
                    f"extentPA{int(vy_cm)}cm_shield{int(shield_mm)}mm_paddle{int(paddle_thickness_mm)}mm_"
...
if compress_option == 'yes':
    ...
    output_file2 = (f"MLO_phantom_vox{int(voxel_size_um)}um_thick{int(d_cm)}cm_extentML{int(vx_cm)}cm_"
                    f"extentPA{int(vy_cm)}cm_shield{int(shield_mm)}mm_paddle{int(paddle_thickness_mm)}mm_"
